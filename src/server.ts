  ...
  socket.on('post:new', async (postData) => {
    try {
      ...
      (async () => {
        ...
        const councilQueue = [];
        councilQueue.push({ name: "GEMINI", type: "flash" });
        councilQueue.push({ name: "DEEPSEEK", type: "chat" });
        if (isBasicPlus) councilQueue.push({ name: "GROK", type: "beta" });
        if (isFullCouncil) {
          councilQueue.push({ name: "CLAUDE", type: "opus" });
          councilQueue.push({ name: "GPT_4", type: "4o" });
        }
        ...
        for (const ai of councilQueue) {
          ...
          const context = transcript.map(p => `${p.is_human ? 'User' : p.name}: ${p.content}`).join("\n\n");

          try {
            let aiContent = "";
            if (ai.name === "GEMINI") {
              const res = await genAI.getGenerativeModel({ model: "gemini-1.5-flash" }).generateContent(context);
              ...
              const res = await openai.chat.completions.create({ model: "grok-beta", messages: [{ role: "user", content: context }] });
              aiContent = res.choices[0].message.content || "";
            } else if (ai.name === "CLAUDE") {
              const res = await anthropic.messages.create({ model: "claude-3-opus-20240229", max_tokens: 1024, messages: [{ role: "user", content: context }] });
              aiContent = (res.content[0] as any).text;
            } else if (ai.name === "GPT_4") {
              const res = await openai.chat.completions.create({ model: "gpt-4o", messages: [{ role: "user", content: context }] });
              aiContent = res.choices[0].message.content || "";
            }
