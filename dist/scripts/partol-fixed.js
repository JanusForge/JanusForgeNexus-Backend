...
async function getCouncilVotes(topics) {
...
async function getGrokVote(topicTitles) {
...
async function getClaudeVote(topicTitles) {
...
${topicTitles.map((t, i) => `${i + 1}. "${t}"`).join('\n')}`;
    ...
    const vote = response.content[0].text.trim();
    return topicTitles.find(t => vote.includes(t)) || topicTitles[0];
}
async function getDeepSeekVote(topicTitles) {
...
async function generateOpeningThoughts(topic) {
...
async function getGrokOpeningThought(topic) {
...
async function getClaudeOpeningThought(topic) {
...
async function getDeepSeekOpeningThought(topic) {
